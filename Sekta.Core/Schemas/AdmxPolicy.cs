...
public class AdmxPolicy
{
    ...
    public static List<AdmxPolicy> From(
        List<PolicyDefinition> rawPolicyList,
        List<AdmxCategory> categories
    )
    {
        return rawPolicyList
            .Select(
                (rp) =>
                {
                    AdmxCategory category = categories.First(
                        (c) => c.Name == rp.ParentCategory.ReferenceName
                    );
