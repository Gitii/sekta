    ...
    public class CheckboxPresentationModelView: BasePresentationModeView
    {
        ...
        public override PolicyOptionValue Serialize(BaseElement[] elements)
        {
            ...
            if (element is BooleanElement booleanElement)
            {
                if (IsChecked)
                {
                    return booleanElement.trueValue.AsPolicyOption(booleanElement.key ?? _admxPolicy.Key, booleanElement.valueName, booleanElement.id);
                }
                else
                {
                    return booleanElement.falseValue.AsPolicyOption(booleanElement.key ?? _admxPolicy.Key, booleanElement.valueName, booleanElement.id);
                }
            }
            else
            {
