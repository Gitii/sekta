...
public class ConfiguredPolicy : ReactiveObject
{
    public ConfiguredPolicy(string policyDefinitionName, PolicyClass policyClass)
    {
        PolicyDefinitionName =
            policyDefinitionName ?? throw new ArgumentNullException(nameof(policyDefinitionName));
    ...
    {
        return JsonConvert.DeserializeObject<List<ConfiguredPolicy>>(
            strPolicyList,
            new ConfiguredPolicyConverter()
        );
    }
    ...
    private class ConfiguredPolicyConverter : JsonConverter<ConfiguredPolicy>
    {
        ...
        public override ConfiguredPolicy ReadJson(
            JsonReader reader,
            Type objectType,
            ConfiguredPolicy existingValue,
        ...
        )
        {
            ...
            if (!version.HasValue || version < MINIMUM_SUPPORTED_VERSION)
            {
                throw new ArgumentException(
                    $"Failed to deserialize the policy: The minimum supported version is {MINIMUM_SUPPORTED_VERSION} but got {version}!"
                );
            }
            ...
            var policy =
                existingValue ?? new ConfiguredPolicy(name, (Admx.Schema.PolicyClass)policyClass);
