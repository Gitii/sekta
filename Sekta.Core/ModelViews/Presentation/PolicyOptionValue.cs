...
namespace Sekta.Core.ModelView.Presentation
{
    public static class PolicyOptionsValueExtensions {
        public static PolicyOptionValue AsPolicyOption(this ValueContainer valueContainer, string path, string keyName, string id)
        {
    ...
    public readonly struct PolicyOptionValue : IEquatable<PolicyOptionValue>
    {
        ...
        private PolicyOptionValue(string path, string keyName, uint? keyValueUSignedInteger, string keyValueString, KeyValuePair<string, string>[] keyValueList, string id)
        {
            Path = path ?? throw new ArgumentNullException(nameof(path));
            KeyName = keyName;
            if (keyName == null && keyValueList == null)
            {
                throw new ArgumentNullException(nameof(keyName), "KeyName can only be null if a key value list is used.");
            }
        ...
        public PolicyOptionValue(string path, string keyName, uint keyValueUSignedInteger, string id): this(path, keyName, keyValueUSignedInteger, null, null, id)
        {
        }

        public PolicyOptionValue(string path, string keyName, string keyValueString, string id): this(path, keyName, null, keyValueString, null, id)
        {
        ...
        public PolicyOptionValue(string path, string keyValuePrefix, KeyValuePair<string, string>[] keyValueList, string id): this(path, keyValuePrefix, null, null, keyValueList, id)
        {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                hashCode = (hashCode * 397) ^ (KeyValueString != null ? KeyValueString.GetHashCode() : 0);
        ...
        {
            SerializedValueContainer container = JsonConvert.DeserializeObject<SerializedValueContainer>(serializedValue);

            switch (container.SerializedValueType)
            {
                case ValueType.String:
                    string strValue = container.SerializedValue;
                    return new PolicyOptionValue(container.Path, container.KeyName, strValue, container.Id);
                case ValueType.Uint:
                    uint uintValue = uint.Parse(container.SerializedValue);
                    return new PolicyOptionValue(container.Path, container.KeyName, uintValue, container.Id);
                case ValueType.KeyValueList:
                    KeyValuePair<string, string>[] list = JsonConvert.DeserializeObject<KeyValuePair<string, string>[]>(container.SerializedValue);
