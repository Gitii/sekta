...
public class PolicyConfigurationModelView : ReactiveObject
{
    ...
    public PolicyConfigurationModelView(
        AdmxPolicy policy,
    ...
    )
    {
        ...
        _presentationElements = (Presentation?.Items ?? Array.Empty<BaseDataElement>())
            .Select<BaseDataElement, BasePresentationModeView>(
                (i) =>
                {
        ...
        foreach (BasePresentationModeView element in _presentationElements)
        {
            element.Changed.Subscribe(
                ...
                {
                    if (
                        e.PropertyName != nameof(BasePresentationModeView.CurrentResources)
            ...
            .Select(
                (
                    (tuple) =>
                    {
                        ...
                        if (
                            validationToken.Count == 0
                            || isEnabled.GetValueOrDefault(false) == false
            ...
            .Select(
                (
                    (isValid) =>
                    {
                        ...
                        {
                            return _presentationElements
                                .FirstOrDefault(pe => pe.ValidationContext != null)
                                ?.ValidationContext.Text.ToSingleLine();
                        }
                    }
                )
            )
    ...
    private async Task ApplyChanges()
    {
        ...
        {
            foreach (
                BasePresentationModeView element in _presentationElements.Where(
                    (pe) => !pe.IsStatic
                )
            )
            {
    ...
    private void AddDisabledValues()
    {
        ...
        {
            _configuredPolicy.Values.Add(
                new ConfiguredPolicyOption(
                    _policy.Key + "_disabledValue",
                    _policy.DisabledValue.AsPolicyOption(
                        _policy.Key,
                        _policy.ValueName,
        ...
        if (_policy.DisabledList != null)
        {
            foreach (ValueItem item in _policy.DisabledList.Items)
            {
                _configuredPolicy.Values.Add(
                    new ConfiguredPolicyOption(
                        _policy.Key + "_disabledList_" + item.Key,
                        item.Value.AsPolicyOption(
                            item.Key,
                            item.ValueName,
                            AdmxPolicy.POLICY_EMBEDDED_OPTION_ID
                        )
    ...
    private void AddEnabledValues()
    {
        if (_policy.EnabledValue != null)
        {
            _configuredPolicy.Values.Add(
                new ConfiguredPolicyOption(
                    _policy.Key + "_enabledValue",
                    _policy.EnabledValue.AsPolicyOption(
                        _policy.Key,
                        _policy.ValueName,
                        AdmxPolicy.POLICY_EMBEDDED_OPTION_ID
                    )
        ...
        if (_policy.EnabledList != null)
        {
            foreach (ValueItem item in _policy.EnabledList.Items)
            {
                _configuredPolicy.Values.Add(
                    new ConfiguredPolicyOption(
                        _policy.Key + "_enabledList_" + item.Key,
                        item.Value.AsPolicyOption(
                            item.Key,
                            item.ValueName,
                            AdmxPolicy.POLICY_EMBEDDED_OPTION_ID
    ...
    private Task RevertChanges()
    {
        ...
        foreach (
            BasePresentationModeView element in _presentationElements.Where((pe) => !pe.IsStatic)
        )
        {
            ConfiguredPolicyOption value = _configuredPolicy.Values.FirstOrDefault(
                (sv) => sv.ElementId == element.Id
            );
