    ...
    public partial class PolicyConfigurationView
    {
        ...
        public PolicyConfigurationView()
        {
            ...
            this.WhenActivated((disposable) =>
            {
                ...
                this.WhenAnyValue(x => x.EnabledRadioButton.IsChecked, x => x.DisabledRadioButton.IsChecked, x => x.NotConfiguredRadioButton.IsChecked)
                    .Subscribe(values =>
                    {
                        ...
                        if (isEnabled.GetValueOrDefault() && ViewModel.IsEnabled.GetValueOrDefault() != true)
                        {
                            ViewModel.IsEnabled = true;
                            ViewModel.IsDataChanged = true;
                        }

                        if (isDisabled.GetValueOrDefault() && ViewModel.IsEnabled.GetValueOrDefault(true) != false)
                        {
                ...
                this.OneWayBind(ViewModel,
                    (vm) => vm.ValidationText,
                    (v) => v.ValidationText.Text
                ...
                this.BindCommand(ViewModel,
                    (vm) => vm.ApplyChangesCommand,
                    (v) => v.ApplyButton
                ).DisposeWith(disposable);

                this.BindCommand(ViewModel,
                    (vm) => vm.RevertChangesCommand,
                    (v) => v.RevertButton
                ).DisposeWith(disposable);
